...
interface AnimatedBulbComponentProps {
  size?: SizeType
}

export const AnimatedBulb = ({
  size = 'medium',
}: AnimatedBulbComponentProps) => {
  ...
  useEffect(() => {
    ...
    > = {
      small: { width: 24, height: 24, scale: 1 },
      medium: { width: 64, height: 64, scale: 1 },
      large: { width: 96, height: 96, scale: 1.5 },
    }
    ...
    // Filament - Use MeshPhongMaterial for emissive properties
    ...
    const filament1Geometry = new THREE.BoxGeometry(
      0.02 * dimensions.scale,
      1.5 * dimensions.scale,
      0.02 * dimensions.scale
    )
    ...
    const animate = () => {
      ...
      if (bulbRef.current && bulbMesh) {
        ...
        bulbRef.current.position.y =
          Math.sin(time * 2) * 0.15 * dimensions.scale
        ...
        for (let i = 0; i < positions.count; i++) {
          ...
          const wave3 =
            Math.sin(time * 2.5 + z * 2.5) * 0.025 * dimensions.scale
